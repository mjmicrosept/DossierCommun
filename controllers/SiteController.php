...
class SiteController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        $searchModelResultat = null;
        $dataProviderResultat = null;
                ...
                $defaultColumns = [
                    ...
                    [
                        ...
                        'value' => function($model){
                            ...
                            $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->andFilterWhere(['context'=>Alerte::CONTEXT_DOCUMENT])->one();
                            if(is_null($aAlerte))
                                return '';
                            else{
                                if(count($aAlerte) == 0)
                                    ...
                                    if($aAlerte->vue == 0){
                                        switch($aAlerte->type){
                                            case Alerte::TYPE_NODOC :
                                                return '<i class="fas fa-exclamation-triangle fa-2x text-red"></i>';
                                                break;
                                            case Alerte::TYPE_PERIODE_MISSING :
                                                return '<i class="fas fa-exclamation-circle fa-2x text-orange"></i>';
                                                break;
                                            case Alerte::TYPE_SENDMAIL :
                                                return '<i class="fas fa-envelope-square fa-2x text-orange"></i>';
                                                break;
                                        }
                                    }
                                    else
                                        return '<strong><i class="fas fa-check-circle fa-2x text-green"></i></strong>';
                                }
                        ...
                        'contentOptions' => function ($model, $key, $index, $column) {
                            $idLabo = $model['id_labo'];
                            $idClient = $model['id_client'];
                            $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->one();
                            if(is_null($aAlerte))
        ...
        //Partie Tableau de bord des résultats d'analyses
        if(!Yii::$app->user->isSuperadmin && (User::getCurrentUser()->hasRole([User::TYPE_CLIENT_ADMIN]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER_GROUP]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER]))){
            ...
            $idClient = null;
            $idLabo = null;
            $searchModelResultat = ['monthAlert' => Yii::$app->request->getQueryParam('filter-monthAlert', '1'),];
            ...
            $dataProviderResultat = new ArrayDataProvider([
                'key'=>function($row) {
                    return $row['id_labo'];
                },
                'allModels' => $data,
            ...
            if(User::getCurrentUser()->hasRole([User::TYPE_CLIENT_ADMIN]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER_GROUP])){
                ...
                foreach ($entete as $item) {
                    array_push($gridColumnResultat,$item);
                }
            }
            if( User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER])){
                $entete = [
                ...
                array_push($gridColumnResultat,$entete);
            }
            ...
            if(User::getCurrentUser()->hasRole([User::TYPE_CLIENT_ADMIN]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER_GROUP])){
                $defaultColumns = [
                    [
                        ...
                        'width'=>'150px',
                        'value' => function($model){
                            $nbTotalAnalyses = AnalyseData::find()->andFilterWhere(['id_client'=>$model['id_client']])->andFilterWhere(['id_labo'=>$model['id_labo']])->count();
                            if(!is_null($nbTotalAnalyses))
                                return $nbTotalAnalyses;
                            else
                                return ' - ';
                        }
                    ],
                    [
                        ...
                        'width'=>'150px',
                        'value' => function($model){
                            $lastPushObj = AnalyseData::find()->andFilterWhere(['id_client'=>$model['id_client']])->andFilterWhere(['id_labo'=>$model['id_labo']])->orderBy('date_analyse DESC')->one();
                            if(!is_null($lastPushObj)) {
                                $lastPush = $lastPushObj->date_analyse;
                                $year = substr($lastPush, 0, 4);
                                $month = intval(substr($lastPush, 5, 2));
                                $day = substr($lastPush, 8, 2);
                                $hour = substr($lastPush, -8, 2);
                    ...
                    [
                        ...
                        'value' => function($model){
                            $lastPushObj = AnalyseData::find()->andFilterWhere(['id_client'=>$model['id_client']])->andFilterWhere(['id_labo'=>$model['id_labo']])->orderBy('date_analyse DESC')->one();
                            if(!is_null($lastPushObj)) {
                                $dateanalyse = $lastPushObj->date_analyse;
                                $nbAnalysesADate = AnalyseData::find()->andFilterWhere(['id_client'=>$model['id_client']])->andFilterWhere(['id_labo'=>$model['id_labo']])->andFilterWhere(['date_analyse'=>$dateanalyse])->all();
                                return count($nbAnalysesADate);
                            }
                            else
                                return ' - ';
                        },
                    ...
                    [
                        ...
                        'value' => function($model){
                            $idLabo = $model['id_labo'];
                            $idClient = $model['id_client'];
                            $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->andFilterWhere(['context'=>Alerte::CONTEXT_ANALYSE])->one();
                            if(is_null($aAlerte))
                                return '';
                            else{
                                if(count($aAlerte) == 0)
                        ...
                        'contentOptions' => function ($model, $key, $index, $column) {
                            ...
                            $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->one();
                            if(is_null($aAlerte))
                                return ['class'=>'idlabo-'.$model['id_client'].'-check-analyse','data-idalerte'=>''];
                            else{
                                if(count($aAlerte) == 0)
                                    return ['class'=>'idlabo-'.$model['id_client'].'-check-analyse','data-idalerte'=>''];
                                else
                                    return ['class'=>'idlabo-'.$model['id_client'].'-check-analyse','data-idalerte'=>$aAlerte->id];
                            }
                    ...
                    [
                        ...
                        'dropdownOptions' => ['class' => 'float-left btn-actions'],
                        'dropdownMenu' => ['style'=>'left:-120px !important;'],
                        'template' => '{periodeanalyse} {noanalyse} {mailadminanalyse} {deletealerteanalyse}',
                        'urlCreator' => function($action, $model, $key, $index) { return '#'; },
                        'viewOptions' => ['title' => 'This will launch the book details page. Disabled for this demo!', 'data-toggle' => 'tooltip'],
                        'updateOptions' => ['title' => 'This will launch the book update page. Disabled for this demo!', 'data-toggle' => 'tooltip'],
                        'deleteOptions' => ['title' => 'This will launch the book delete action. Disabled for this demo!', 'data-toggle' => 'tooltip'],
                        'buttons'=>[
                            'periodeanalyse' => function ($url, $model, $key) {
                                $nbAnalyseTotal = AnalyseData::find()->andFilterWhere(['id_client'=>$model['id_client']])->andFilterWhere(['id_labo'=>$model['id_labo']])->count();
                                if($nbAnalyseTotal == 0)
                                    return '';
                                else {
                                    $idLabo = $model['id_labo'];
                                    ...
                                    $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->andFilterWhere(['context'=>Alerte::CONTEXT_ANALYSE])->one();
                                    if(is_null($aAlerte))
                                        return '<li class="li-alerte liperiodeanalyse-'.$model['id_client'].'"><span class="periode-analyse-alerte span-alerte" data-etablissement="' . $model['id_client'] . '" data-labo="' . $model['id_labo'] . '" title="Période sans analyses"><span class="glyphicon glyphicon-time" style="margin-right:10px;"></span> Période sans analyses</span></li>';
                                    else
                                        return '';
                                }
                            },
                            'noanalyse' => function ($url, $model, $key) {
                                $nbAnalyseTotal = AnalyseData::find()->andFilterWhere(['id_client'=>$model['id_client']])->andFilterWhere(['id_labo'=>$model['id_labo']])->count();
                                if($nbAnalyseTotal == 0) {
                                    $idLabo = $model['id_labo'];
                                    $idClient = $model['id_client'];
                                    $aAlerte = Alerte::find()->andFilterWhere(['id_labo' => $idLabo])->andFilterWhere(['id_etablissement' => $idClient])->andFilterWhere(['active' => 1])->andFilterWhere(['context'=>Alerte::CONTEXT_ANALYSE])->one();
                                    if (is_null($aAlerte))
                                        return '<li class="li-alerte linodocanalyse-'.$model['id_client'].'"><span class="no-analyse-alerte span-alerte" data-etablissement="' . $model['id_client'] . '" data-labo="' . $model['id_labo'] . '" title="Pas d\'analyses pour ce laboratoire"><span class="glyphicon glyphicon-level-up" style="margin-right:10px;"></span> Pas d\'analyses</span></li>';
                                    else
                                        return '';
                                }
                                else
                            ...
                            'mailadminanalyse' => function ($url, $model, $key) {
                                $idLabo = $model['id_labo'];
                                $idClient = $model['id_client'];
                                $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->andFilterWhere(['context'=>Alerte::CONTEXT_ANALYSE])->one();
                                if(is_null($aAlerte))
                                    return '<li class="li-alerte limailadminanalyse-'.$model['id_client'].'"><span class="mailadmin-analyse-alerte span-alerte" data-etablissement="'.$model['id_client'].'" data-labo="'.$model['id_labo'].'" title="Envoyer un mail au laboratoire"><span class="glyphicon glyphicon-envelope" style="margin-right:10px;"></span> Envoyer un mail</span></li>';
                                else
                                    return '';
                            },
                            'deletealerteanalyse' => function ($url, $model, $key) {
                                $idLabo = $model['id_labo'];
                                $idClient = $model['id_client'];
                                $aAlerte = Alerte::find()->andFilterWhere(['id_labo'=>$idLabo])->andFilterWhere(['id_etablissement'=>$idClient])->andFilterWhere(['active'=>1])->andFilterWhere(['context'=>Alerte::CONTEXT_ANALYSE])->one();
                                if(is_null($aAlerte))
                                    return '<li class="li-alerte lialerteanalyse-'.$model['id_client'].'" style="pointer-events:none;"><span class="deletealerte-analyse-alerte deletealerteanalyse-'.$model['id_client'].' span-alerte" data-etablissement="'.$model['id_client'].'" data-labo="'.$model['id_labo'].'" title="Supprimer l\'alerte"><span class="glyphicon glyphicon-trash" style="margin-right:10px;"></span> Supprimer l\'alerte</span></li>';
                                else
                                    return '<li class="li-alerte lialerteanalyse-'.$model['id_client'].'" style="pointer-events:auto;"><span class="deletealerte-analyse-alerte deletealerteanalyse-'.$model['id_client'].' span-alerte" data-etablissement="'.$model['id_client'].'" data-labo="'.$model['id_labo'].'" data-idalerte="'.$aAlerte->id.'" title="Supprimer l\'alerte"><span class="glyphicon glyphicon-trash" style="margin-right:10px;"></span> Supprimer l\'alerte</span></li>';
                            },
                        ],
                        'headerOptions' => ['class' => 'kartik-sheet-style'],
                    ],
        ...
        //Partie tableau de bord des analyses non conformes
        if(!Yii::$app->user->isSuperadmin && (User::getCurrentUser()->hasRole([User::TYPE_CLIENT_ADMIN]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER_GROUP]) || User::getCurrentUser()->hasRole([User::TYPE_CLIENT_USER]))){
