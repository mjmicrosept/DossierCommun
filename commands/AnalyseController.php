...
class AnalyseController extends Controller
{
    ...
    public function actionImportation($jour=null, $annee=null)
    {
        ...
        try {
            $ftp = new FtpClient();
            $ftp->connect(\Yii::$app->params['analyses']['host']);
            $ftp->login(\Yii::$app->params['analyses']['login'], \Yii::$app->params['analyses']['password']);
            $ftp->pasv(true);
                    ...
                    if(!is_null($client)) {
                        if($client->active == 1 && $client->is_analyzable == 1) {
                            ...
                            if(!is_null($clientMap)) {
                                $remote_file = \Yii::$app->params['analyses']['fileAnalyse'];
